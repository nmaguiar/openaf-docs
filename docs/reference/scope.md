...
````
Builds an object to allow access through ssh. aMap should be a ssh string with the format: ssh://user:pass@host:port/identificationKey?timeout=1234&compression=true or a map with the keys: host, port, login, pass, id, compress and timeout. See "help SSH.SSH" for more info.
````
### $ssh.$ssh
...
__oJobRunJob(aJob, args, aId) : boolean__
