...
| DB | H2 | [12](#12) | Perform a SQL query over a H2 database. |
| DB | H2 | [13](#13) | Store the json result of a command into a H2 database table. |
| DB | SQLite | [14](#14) | Perform a query over a database using JDBC. |
| DB | SQLite | [15](#15) | Store the json result on a SQLite database table. |
| Docker | Containers | [16](#16) | Output a table with the list of running containers. |
| Docker | Network | [17](#17) | Output a table with the docker networks info. |
| Docker | Stats | [18](#18) | Output a table with the docker stats broken down for each value. |
| Docker | Storage | [19](#19) | Output a table with the docker volumes info. |
| ElasticSearch | Cluster | [20](#20) | Get an ElasticSearch/OpenSearch cluster nodes overview |
| ElasticSearch | Cluster | [21](#21) | Get an ElasticSearch/OpenSearch cluster per host data allocation |
| ElasticSearch | Cluster | [22](#22) | Get an ElasticSearch/OpenSearch cluster settings flat |
| ElasticSearch | Cluster | [23](#23) | Get an ElasticSearch/OpenSearch cluster settings non-flatted |
| ElasticSearch | Cluster | [24](#24) | Get an ElasticSearch/OpenSearch cluster stats per node |
| ElasticSearch | Cluster | [25](#25) | Get an overview of an ElasticSearch/OpenSearch cluster health |
| ElasticSearch | Indices | [26](#26) | Get an ElasticSearch/OpenSearch count per index |
| ElasticSearch | Indices | [27](#27) | Get an ElasticSearch/OpenSearch indices overview |
| ElasticSearch | Indices | [28](#28) | Get an ElasticSearch/OpenSearch settings for a specific index |
| Generic | Excel | [29](#29) | Building an Excel file with the AWS IPv4 and IPv6 ranges (1). |
| Generic | Excel | [30](#30) | Building an Excel file with the AWS IPv4 and IPv6 ranges (2). |
| Generic | Excel | [31](#31) | Building an Excel file with the AWS IPv4 and IPv6 ranges (3). |
| Generic | Excel | [32](#32) | Processes each json file in /some/data creating and updating the data.xlsx file with a sheet for each file. |
| Generic | Excel | [33](#33) | Store and retrieve data from an Excel spreadsheet |
| Generic | RSS | [34](#34) | Example of generating a HTML list of titles, links and publication dates from a RSS feed |
| Generic | Text | [35](#35) | Get a json with lyrics of a song. |
| GitHub | Releases | [36](#36) | Builds a table of GitHub project releases |
| GitHub | Releases | [37](#37) | Parses the latest GitHub project release markdown notes |
| Grid | Java | [38](#38) | Parse a Java stacktrace into a looping grid. |
| Grid | Kubernetes | [39](#39) | Displays a continuous updating grid with a line chart with the number of CPU throtlles and bursts recorded in the Linux cgroup cpu stats of a container running in Kubernetes and the source cpu.stats data |
| Grid | Unix | [40](#40) | Output a chart with the current Unix load using uptime |
| JSON Schemas | Lists | [41](#41) | Get a list of JSON schemas from Schema Store catalog |
| Kubernetes | Containers | [42](#42) | Parse the Linux cgroup cpu stats on a container running in Kubernetes |
| Kubernetes | Kubectl | [43](#43) | Build an output table with Kubernetes pods with namespace, pod name, container name and corresponding resources using kubectl |
| Kubernetes | Kubectl | [44](#44) | Build an output table with Kubernetes pods with node, namespace, pod name, container name and corresponding resources using kubectl |
| Kubernetes | Kubectl | [45](#45) | List of Kubernetes CPU, memory and storage stats per node using kubectl |
| Kubernetes | Kubectl | [46](#46) | List of Kubernetes pods per namespace and kind using kubectl |
| Mac | Brew | [47](#47) | List all the packages and corresponding versions installed in a Mac by brew. |
| Mac | Info | [48](#48) | Parses the current Mac OS overview information |
| OpenAF | Channels | [49](#49) | Copy the json result of a command into an etcd database using OpenAF&#x27;s channels |
| OpenAF | Channels | [50](#50) | Getting all data stored in an etcd database using OpenAF&#x27;s channels |
| OpenAF | Channels | [51](#51) | Perform a query to a metric &amp; label, with a start and end time, to a Prometheus server using OpenAF&#x27;s channels |
| OpenAF | Channels | [52](#52) | Retrieve all keys stores in a H2 MVStore file using OpenAF&#x27;s channels |
| OpenAF | Channels | [53](#53) | Store and retrieve data from a Redis database |
| OpenAF | Channels | [54](#54) | Store and retrieve data from a RocksDB database |
| OpenAF | Channels | [55](#55) | Store the json results of a command into a H2 MVStore file using OpenAF&#x27;s channels |
| OpenAF | Network | [56](#56) | List all network addresses returned from the current DNS server for a hostname using OpenAF |
| OpenAF | OS | [57](#57) | Current OS information visible to OpenAF |
| OpenAF | TLS | [58](#58) | List the TLS certificates of a target host with a sorted alternative names using OpenAF |
| OpenAF | oPacks | [59](#59) | Listing all currently accessible OpenAF&#x27;s oPacks |
| OpenAF | oafp | [60](#60) | Filter the OpenAF&#x27;s oafp examples list by a specific word in the description |
| OpenAF | oafp | [61](#61) | List the OpenAF&#x27;s oafp examples by category, sub-category and description |
| Unix | Alpine | [62](#62) | List all installed packages in an Alpine system |
| Unix | Compute | [63](#63) | Parses the Linux /proc/cpuinfo into an array |
| Unix | Debian/Ubuntu | [64](#64) | List all installed packages in a Debian/Ubuntu system |
| Unix | Files | [65](#65) | Converting the Linux&#x27;s /etc/os-release to SQL insert statements. |
| Unix | Files | [66](#66) | Converting the Unix&#x27;s syslog into a json output. |
| Unix | Files | [67](#67) | Parses the Linux /etc/passwd to a table order by uid and gid. |
| Unix | Generic | [68](#68) | Creates, in unix, a data.ndjson file where each record is formatted from json files in /some/data |
| Unix | Network | [69](#69) | Loop over the current Linux active network connections |
| Unix | Network | [70](#70) | Parse the Linux &#x27;arp&#x27; command output |
| Unix | Network | [71](#71) | Parse the Linux &#x27;ip tcp_metrics&#x27; command |
| Unix | Network | [72](#72) | Parse the result of the Linux route command |
| Unix | OpenSuse | [73](#73) | List all installed packages in an OpenSuse system or zypper based system |
| Unix | RedHat | [74](#74) | List all installed packages in a RedHat system or rpm based system (use rpm --querytags to list all fields available) |
| Unix | Storage | [75](#75) | Converting the Unix&#x27;s df output |
| Unix | Storage | [76](#76) | Parses the result of the Unix ls command |
| Unix | SystemCtl | [77](#77) | Converting the Unix&#x27;s systemctl list-timers |
| Unix | SystemCtl | [78](#78) | Converting the Unix&#x27;s systemctl list-units |
| Unix | SystemCtl | [79](#79) | Converting the Unix&#x27;s systemctl list-units into an overview table |
| Unix | UBI | [80](#80) | List all installed packages in an UBI system |
| Windows | Network | [81](#81) | Output a table with the current route table using Windows&#x27; PowerShell |
| Windows | Network | [82](#82) | Output a table with the list of network interfaces using Windows&#x27; PowerShell |
| Windows | PnP | [83](#83) | Output a table with USB/PnP devices using Windows&#x27; PowerShell |
| Windows | Storage | [84](#84) | Output a table with the attached disk information using Windows&#x27; PowerShell |
