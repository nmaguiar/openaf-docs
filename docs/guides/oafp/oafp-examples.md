...
| Azure | Bing | [32](#32) | Given an Azure Bing Search API key and a query returns the corresponding search result from Bing. |
| Channels | S3 | [33](#33) | Given a S3 bucket will load data from a previously store data in a provided prefix |
| Channels | S3 | [34](#34) | Given a S3 bucket will save a list of data (the current list of name and versions of OpenAF&#x27;s oPacks) within a provided prefix |
| Chart | Unix | [35](#35) | Output a chart with the current Unix load using uptime |
| DB | H2 | [36](#36) | Perform a SQL query over a H2 database. |
| DB | H2 | [37](#37) | Perform queries and DML SQL statements over a H2 databases |
| DB | H2 | [38](#38) | Store the json result of a command into a H2 database table. |
| DB | List | [39](#39) | List all OpenAF&#x27;s oPack pre-prepared JDBC drivers |
| DB | Mongo | [40](#40) | List all records from a specific MongoDB database and collection from a remote Mongo database. |
| DB | SQLite | [41](#41) | Lists all files in openaf.jar, stores the result in a &#x27;data&#x27; table on a SQLite data.db file and performs a query over the stored data. |
| DB | SQLite | [42](#42) | Perform a query over a database using JDBC. |
| DB | SQLite | [43](#43) | Store the json result on a SQLite database table. |
| Diff | Envs | [44](#44) | Given two JSON files with environment variables performs a diff and returns a colored result with the corresponding differences |
| Diff | Lines | [45](#45) | Performing a diff between two long command lines to spot differences |
| Diff | Path | [46](#46) | Given two JSON files with the parsed PATH environment variable performs a diff and returns a colored result with the corresponding differences |
| Docker | Containers | [47](#47) | Output a table with the list of running containers. |
| Docker | Listing | [48](#48) | List all containers with the docker-compose project, service name, file, id, name, image, creation time, status, networks and ports. |
| Docker | Listing | [49](#49) | List all containers with their corresponding labels parsed and sorted. |
| Docker | Network | [50](#50) | Output a table with the docker networks info. |
| Docker | Registry | [51](#51) | List all a table of docker container images repository and corresponding tags of a private registry. |
| Docker | Registry | [52](#52) | List all the docker container image repositories of a private registry. |
| Docker | Registry | [53](#53) | List all the docker container image repository tags of a private registry. |
| Docker | Stats | [54](#54) | Output a table with the docker stats broken down for each value. |
| Docker | Storage | [55](#55) | Output a table with the docker volumes info. |
| Docker | Volumes | [56](#56) | Given a list of docker volumes will remove all of them, if not in use, in parallel for faster execution. |
| ElasticSearch | Cluster | [57](#57) | Get an ElasticSearch/OpenSearch cluster nodes overview |
| ElasticSearch | Cluster | [58](#58) | Get an ElasticSearch/OpenSearch cluster per host data allocation |
| ElasticSearch | Cluster | [59](#59) | Get an ElasticSearch/OpenSearch cluster settings flat |
| ElasticSearch | Cluster | [60](#60) | Get an ElasticSearch/OpenSearch cluster settings non-flatted |
| ElasticSearch | Cluster | [61](#61) | Get an ElasticSearch/OpenSearch cluster stats per node |
| ElasticSearch | Cluster | [62](#62) | Get an overview of an ElasticSearch/OpenSearch cluster health |
| ElasticSearch | Indices | [63](#63) | Get an ElasticSearch/OpenSearch count per index |
| ElasticSearch | Indices | [64](#64) | Get an ElasticSearch/OpenSearch indices overview |
| ElasticSearch | Indices | [65](#65) | Get an ElasticSearch/OpenSearch settings for a specific index |
| GIT | History | [66](#66) | Give a GIT repository will retrieve the current log history and parse it to an Excel (XLS) file. |
| GPU | Nvidia | [67](#67) | Builds a grid with two charts providing a visualization over a Nvidia GPU usage and the corresponding memory usage for a specific GPU_IDX (gpu index) |
| GPU | Nvidia | [68](#68) | Get current Nvidia per-gpu usage |
| Generic | Arrays | [69](#69) | Converting an array of strings into an array of maps |
| Generic | Avro | [70](#70) | Given an Avro data file outputs it&#x27;s corresponding statistics |
| Generic | Avro | [71](#71) | Given an Avro data file outputs the correspoding schema |
| Generic | Avro | [72](#72) | Reads an Avro data file as input |
| Generic | Avro | [73](#73) | Write an Avro data file as an output |
| Generic | BOM | [74](#74) | Given a container image use syft to generate a table with a corresponding bill-of-materials (BOM) table with each identified artifact name, version, type, language, found path and maven group &amp; id (if applicable). |
| Generic | Base64 | [75](#75) | Encode/decode data (or text-like files) to/from gzip base64 representation for easier packing and transport. |
| Generic | CSV | [76](#76) | Given a BOM CSV with multiple fields (7 in total) return a new CSV with just 3 of the original fields. |
| Generic | Commands | [77](#77) | Given an input array with phone numbers will run parallel output commands, calling ojob.io/telco/phoneNumber, for each entry effectively building an output from those multiple command executions. |
| Generic | End of life | [78](#78) | List the versions of a given product with the corresponding end-of-life dates (using endofline.date API) |
| Generic | Excel | [79](#79) | Building an Excel file with the AWS IPv4 and IPv6 ranges (1). |
| Generic | Excel | [80](#80) | Building an Excel file with the AWS IPv4 and IPv6 ranges (2). |
| Generic | Excel | [81](#81) | Building an Excel file with the AWS IPv4 and IPv6 ranges (3). |
| Generic | Excel | [82](#82) | Processes each json file in /some/data creating and updating the data.xlsx file with a sheet for each file. |
| Generic | Excel | [83](#83) | Store and retrieve data from an Excel spreadsheet |
| Generic | HTML | [84](#84) | Generate a HTML with table of emoticons/emojis by category, group, name, unicode and html code. |
| Generic | Hex | [85](#85) | Outputs an hexadecimal representation of the characters of the file provided allowing to adjust how many per line/row. |
| Generic | List files | [86](#86) | After listing all files and folders recursively producing a count table by file extension. |
| Generic | RSS | [87](#87) | Builds an HTML file with the current linked news titles, publication date and source from Google News RSS. |
| Generic | RSS | [88](#88) | Example of generating a HTML list of titles, links and publication dates from a RSS feed |
| Generic | RSS | [89](#89) | Generates a HTML page with the current news from Google News, order by date, and opens a browser with it. |
| Generic | RSS | [90](#90) | Parses the Slashdot&#x27;s RSS feed news into a quick clickable HTML page in a browser |
| Generic | Set | [91](#91) | Given two json files, with arrays of component versions, generate a table with the difference on one of the sides. |
| Generic | Set | [92](#92) | Given two json files, with arrays of component versions, generate a table with the union of the two sides. |
| Generic | Template | [93](#93) | Given a meal name will search &#x27;The Meal DB&#x27; site for the corresponding recipe and render a markdown HTML of the corresponding recipe. |
| Generic | Text | [94](#94) | Get a json with lyrics of a song. |
| Generic | Text | [95](#95) | Search a word in the English dictionary returning phonetic, meanings, synonyms, antonyms, etc. |
| Generic | YAML | [96](#96) | Given an YAML file with a data array composed of maps with fields &#x27;c&#x27;, &#x27;s&#x27;, &#x27;d&#x27; and &#x27;e&#x27; filter by any record where any field doesn&#x27;t have contents. |
| GitHub | GIST | [97](#97) | Using GitHub&#x27;s GIST functionality retrieves and parses an oAFp examples YAML file with the template and the corresponding data. |
| GitHub | Releases | [98](#98) | Builds a table of GitHub project releases |
| GitHub | Releases | [99](#99) | Parses the latest GitHub project release markdown notes |
| Grid | Java | [100](#100) | Parses a Java hsperf data + the current rss java process memory into a looping grid. |
| Grid | Java | [101](#101) | Parses a Java hsperf data into a looping grid. |
| Grid | Kubernetes | [102](#102) | Displays a continuous updating grid with a line chart with the number of CPU throtlles and bursts recorded in the Linux cgroup cpu stats of a container running in Kubernetes and the source cpu.stats data |
| Grid | Mac | [103](#103) | Shows a grid with the Mac network metrics and 4 charts for in, out packets and in, out bytes |
| Grid | Mac | [104](#104) | Shows a grid with the Mac storage metrics and 4 charts for read, write IOPS and read, write bytes per second |
| Grid | Unix | [105](#105) | On an Unix/Linux system supporting &#x27;ps&#x27; output formats %cpu and %mem, will output a chart with the percentage of cpu and memory usage of a provided pid (e.g. 12345) |
| JSON Schemas | Lists | [106](#106) | Get a list of JSON schemas from Schema Store catalog |
| Java | Certificates | [107](#107) | Given a Java keystore will obtain a list of certificates and output them order by the ones that will expire first. |
| Kubernetes | Containers | [108](#108) | Parse the Linux cgroup cpu stats on a container running in Kubernetes |
| Kubernetes | Kubectl | [109](#109) | Build an output table with Kubernetes pods with namespace, pod name, container name and corresponding resources using kubectl |
| Kubernetes | Kubectl | [110](#110) | Build an output table with Kubernetes pods with node, namespace, pod name, container name and corresponding resources using kubectl |
| Kubernetes | Kubectl | [111](#111) | Executes a recursive file list find command in a specific pod, namespace and path converting the result into a table. |
| Kubernetes | Kubectl | [112](#112) | Given the list of all Kubernetes objects will produce a list of objects per namespace, kind, apiVersiom, creation timestamp, name and owner. |
| Kubernetes | Kubectl | [113](#113) | List of Kubernetes CPU, memory and storage stats per node using kubectl |
| Kubernetes | Kubectl | [114](#114) | List of Kubernetes pods per namespace and kind using kubectl |
| Kubernetes | Kubectl | [115](#115) | Produces a list of pods&#x27; containers per namespace with the corresponding images and assigned nodes. |
| Kubernetes | PVC | [116](#116) | Produces a table with all Kubernetes persistent volume claims (PVCs) in use by pods. |
| Mac | Activity | [117](#117) | Uses the Mac terminal command &#x27;last&#x27; output to build an activity table with user, tty, from, login-time and logout-time |
| Mac | Brew | [118](#118) | List all the packages and corresponding versions installed in a Mac by brew. |
| Mac | Chart | [119](#119) | On a Mac OS produce a looping chart with the total percentage of current CPU usage. |
| Mac | Info | [120](#120) | Get a list of the current logged users in Mac OS |
| Mac | Info | [121](#121) | Parses the current Mac OS hardware information |
| Mac | Info | [122](#122) | Parses the current Mac OS overview information |
| Mac | Safari | [123](#123) | Get a list of all Mac OS Safari bookmarks into a CSV file. |
| Mac | Tunnelblink | [124](#124) | In a Mac OS with Tunnelblink, if you want to copy all your OpenVPN configurations into ovpn files. |
| Markdown | Tables | [125](#125) | For an input markdown file, parse all tables, transform it to JSON and output as a colored table |
| Network | ASN | [126](#126) | Retrieve an IP to ASN list list and converts it to ndjson |
| Network | ASN | [127](#127) | Retrieve the list of ASN number and names from RIPE and transforms it to a CSV. |
| Network | Latency | [128](#128) | Given a host and a port will display a continuously updating line chart with network latency, in ms, between the current device and the target host and port |
| Ollama | List models | [129](#129) | Parses the list of models currently in an Ollama deployment |
| OpenAF | Channels | [130](#130) | Copy the json result of a command into an etcd database using OpenAF&#x27;s channels |
| OpenAF | Channels | [131](#131) | Getting all data stored in an etcd database using OpenAF&#x27;s channels |
| OpenAF | Channels | [132](#132) | Given a Prometheus database will query for a specific metric (go_memstats_alloc_bytes), during a defined period, every 5 seconds (step) will produce a static chart with the corresponding metric values. |
| OpenAF | Channels | [133](#133) | Perform a query to a metric &amp; label, with a start and end time, to a Prometheus server using OpenAF&#x27;s channels |
| OpenAF | Channels | [134](#134) | Retrieve all keys stores in a H2 MVStore file using OpenAF&#x27;s channels |
| OpenAF | Channels | [135](#135) | Store and retrieve data from a Redis database |
| OpenAF | Channels | [136](#136) | Store and retrieve data from a RocksDB database |
| OpenAF | Channels | [137](#137) | Store the json results of a command into a H2 MVStore file using OpenAF&#x27;s channels |
| OpenAF | Flags | [138](#138) | List the current values of OpenAF/oAFp internal flags |
| OpenAF | Network | [139](#139) | Gets all the DNS host addresses for a provided domain and ensures that the output is always a list |
| OpenAF | Network | [140](#140) | List all MX (mail servers) network addresses from the current DNS server for a hostname using OpenAF |
| OpenAF | Network | [141](#141) | List all network addresses returned from the current DNS server for a hostname using OpenAF |
| OpenAF | OS | [142](#142) | Current OS information visible to OpenAF |
| OpenAF | OS | [143](#143) | Using OpenAF parse the current environment variables |
| OpenAF | OpenVPN | [144](#144) | Using OpenAF code to perform a more complex parsing of the OpenVPN status data running on an OpenVPN container (nmaguiar/openvpn) called &#x27;openvpn&#x27; |
| OpenAF | SFTP | [145](#145) | Generates a file list with filepath, size, permissions, create and last modified time from a SFTP connection with user and password |
| OpenAF | SFTP | [146](#146) | Generates a file list with filepath, size, permissions, create and last modified time from a SFTP connection with user, private key and password |
| OpenAF | TLS | [147](#147) | List the TLS certificates of a target host with a sorted alternative names using OpenAF |
| OpenAF | oJob.io | [148](#148) | Parses ojob.io/news results into a clickable news title HMTL page. |
| OpenAF | oJob.io | [149](#149) | Retrieves the list of oJob.io&#x27;s jobs and filters which start by &#x27;ojob.io/news&#x27; to display them in a rectangle |
| OpenAF | oPacks | [150](#150) | Listing all currently accessible OpenAF&#x27;s oPacks |
| OpenAF | oafp | [151](#151) | Filter the OpenAF&#x27;s oafp examples list by a specific word in the description |
| OpenAF | oafp | [152](#152) | List the OpenAF&#x27;s oafp examples by category, sub-category and description |
| OpenAF | oafp | [153](#153) | Produce a colored table with all the current oafp input and output formats supported. |
| OpenVPN | List | [154](#154) | When using the container nmaguiar/openvpn it&#x27;s possible to convert the list of all clients order by expiration/end date |
| Unix | Activity | [155](#155) | Uses the Linux command &#x27;last&#x27; output to build a table with user, tty, from and period of activity for Debian based Linuxs |
| Unix | Activity | [156](#156) | Uses the Linux command &#x27;last&#x27; output to build a table with user, tty, from and period of activity for RedHat based Linuxs |
| Unix | Alpine | [157](#157) | List all installed packages in an Alpine system |
| Unix | Ask | [158](#158) | Unix bash script to ask for a path and choose between filetypes to perform an unix find command. |
| Unix | Compute | [159](#159) | Parses the Linux /proc/cpuinfo into an array |
| Unix | Debian/Ubuntu | [160](#160) | List all installed packages in a Debian/Ubuntu system |
| Unix | Envs | [161](#161) | Converts the Linux envs command result into a table of environment variables and corresponding values |
| Unix | Files | [162](#162) | Converting the Linux&#x27;s /etc/os-release to SQL insert statements. |
| Unix | Files | [163](#163) | Converting the Unix&#x27;s syslog into a json output. |
| Unix | Files | [164](#164) | Executes a recursive file list find command converting the result into a table. |
| Unix | Files | [165](#165) | Parses the Linux /etc/passwd to a table order by uid and gid. |
| Unix | Generic | [166](#166) | Creates, in unix, a data.ndjson file where each record is formatted from json files in /some/data |
| Unix | Memory map | [167](#167) | Given an Unix process will output a table with process&#x27;s components memory address, size in bytes, permissions and owner |
| Unix | Network | [168](#168) | Loop over the current Linux active network connections |
| Unix | Network | [169](#169) | Parse the Linux &#x27;arp&#x27; command output |
| Unix | Network | [170](#170) | Parse the Linux &#x27;ip tcp_metrics&#x27; command |
| Unix | Network | [171](#171) | Parse the result of the Linux route command |
| Unix | OpenSuse | [172](#172) | List all installed packages in an OpenSuse system or zypper based system |
| Unix | RedHat | [173](#173) | List all installed packages in a RedHat system or rpm based system (use rpm --querytags to list all fields available) |
| Unix | Storage | [174](#174) | Converting the Unix&#x27;s df output |
| Unix | Storage | [175](#175) | Parses the result of the Unix ls command |
| Unix | SystemCtl | [176](#176) | Converting the Unix&#x27;s systemctl list-timers |
| Unix | SystemCtl | [177](#177) | Converting the Unix&#x27;s systemctl list-units |
| Unix | SystemCtl | [178](#178) | Converting the Unix&#x27;s systemctl list-units into an overview table |
| Unix | UBI | [179](#179) | List all installed packages in an UBI system |
| Unix | named | [180](#180) | Converts a Linux&#x27;s named log, for client queries, into a CSV |
| Unix | strace | [181](#181) | Given a strace unix command will produce a summary table of the system calls invoked including a small line chart of the percentage of time of each. |
| Windows | Network | [182](#182) | Output a table with the current route table using Windows&#x27; PowerShell |
| Windows | Network | [183](#183) | Output a table with the list of network interfaces using Windows&#x27; PowerShell |
| Windows | PnP | [184](#184) | Output a table with USB/PnP devices using Windows&#x27; PowerShell |
| Windows | Storage | [185](#185) | Output a table with the attached disk information using Windows&#x27; PowerShell |
| XML | Maven | [186](#186) | Given a Maven pom.xml parses the XML content to a colored table ordering by the fields groupId and artifactId. |
