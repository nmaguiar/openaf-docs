...
| AWS | DynamoDB | [27](#27) | Given an AWS DynamoDB table &#x27;my-users&#x27; will produce a colored tree output by getting the item for the email key &#x27;scott.tiger@example.com&#x27; |
...
| AWS | Lambda | [31](#31) | Prepares a table of AWS Lambda functions with their corresponding main details |
| AWS | Lambda | [32](#32) | Prepares a table, for a specific AWS Lambda function during a specific time periods, with number of invocations and minimum, average and maximum duration per periods from AWS CloudWatch |
| AWS | RDS Data | [33](#33) | Given an AWS RDS, postgresql based, database with RDS Data API activated execute the &#x27;analyze&#x27; statement for each table on the &#x27;public&#x27; schema. |
| Azure | Bing | [34](#34) | Given an Azure Bing Search API key and a query returns the corresponding search result from Bing. |
| Channels | S3 | [35](#35) | Given a S3 bucket will load data from a previously store data in a provided prefix |
| Channels | S3 | [36](#36) | Given a S3 bucket will save a list of data (the current list of name and versions of OpenAF&#x27;s oPacks) within a provided prefix |
| Chart | Unix | [37](#37) | Output a chart with the current Unix load using uptime |
| DB | H2 | [38](#38) | Perform a SQL query over a H2 database. |
| DB | H2 | [39](#39) | Perform queries and DML SQL statements over a H2 databases |
| DB | H2 | [40](#40) | Store the json result of a command into a H2 database table. |
| DB | List | [41](#41) | List all OpenAF&#x27;s oPack pre-prepared JDBC drivers |
| DB | Mongo | [42](#42) | List all records from a specific MongoDB database and collection from a remote Mongo database. |
| DB | SQLite | [43](#43) | Lists all files in openaf.jar, stores the result in a &#x27;data&#x27; table on a SQLite data.db file and performs a query over the stored data. |
| DB | SQLite | [44](#44) | Perform a query over a database using JDBC. |
| DB | SQLite | [45](#45) | Store the json result on a SQLite database table. |
| Diff | Envs | [46](#46) | Given two JSON files with environment variables performs a diff and returns a colored result with the corresponding differences |
| Diff | Lines | [47](#47) | Performing a diff between two long command lines to spot differences |
| Diff | Path | [48](#48) | Given two JSON files with the parsed PATH environment variable performs a diff and returns a colored result with the corresponding differences |
| Docker | Containers | [49](#49) | Output a table with the list of running containers. |
| Docker | Listing | [50](#50) | List all containers with the docker-compose project, service name, file, id, name, image, creation time, status, networks and ports. |
| Docker | Listing | [51](#51) | List all containers with their corresponding labels parsed and sorted. |
| Docker | Network | [52](#52) | Output a table with the docker networks info. |
| Docker | Registry | [53](#53) | List all a table of docker container images repository and corresponding tags of a private registry. |
| Docker | Registry | [54](#54) | List all the docker container image repositories of a private registry. |
| Docker | Registry | [55](#55) | List all the docker container image repository tags of a private registry. |
| Docker | Stats | [56](#56) | Output a table with the docker stats broken down for each value. |
| Docker | Storage | [57](#57) | Output a table with the docker volumes info. |
| Docker | Volumes | [58](#58) | Given a list of docker volumes will remove all of them, if not in use, in parallel for faster execution. |
| ElasticSearch | Cluster | [59](#59) | Get an ElasticSearch/OpenSearch cluster nodes overview |
| ElasticSearch | Cluster | [60](#60) | Get an ElasticSearch/OpenSearch cluster per host data allocation |
| ElasticSearch | Cluster | [61](#61) | Get an ElasticSearch/OpenSearch cluster settings flat |
| ElasticSearch | Cluster | [62](#62) | Get an ElasticSearch/OpenSearch cluster settings non-flatted |
| ElasticSearch | Cluster | [63](#63) | Get an ElasticSearch/OpenSearch cluster stats per node |
| ElasticSearch | Cluster | [64](#64) | Get an overview of an ElasticSearch/OpenSearch cluster health |
| ElasticSearch | Indices | [65](#65) | Get an ElasticSearch/OpenSearch count per index |
| ElasticSearch | Indices | [66](#66) | Get an ElasticSearch/OpenSearch indices overview |
| ElasticSearch | Indices | [67](#67) | Get an ElasticSearch/OpenSearch settings for a specific index |
| GIT | History | [68](#68) | Give a GIT repository will retrieve the current log history and parse it to an Excel (XLS) file. |
| GPU | Nvidia | [69](#69) | Builds a grid with two charts providing a visualization over a Nvidia GPU usage and the corresponding memory usage for a specific GPU_IDX (gpu index) |
| GPU | Nvidia | [70](#70) | Get current Nvidia per-gpu usage |
| Generic | Arrays | [71](#71) | Converting an array of strings into an array of maps |
| Generic | Avro | [72](#72) | Given an Avro data file outputs it&#x27;s corresponding statistics |
| Generic | Avro | [73](#73) | Given an Avro data file outputs the correspoding schema |
| Generic | Avro | [74](#74) | Reads an Avro data file as input |
| Generic | Avro | [75](#75) | Write an Avro data file as an output |
| Generic | BOM | [76](#76) | Given a container image use syft to generate a table with a corresponding bill-of-materials (BOM) table with each identified artifact name, version, type, language, found path and maven group &amp; id (if applicable). |
| Generic | Base64 | [77](#77) | Encode/decode data (or text-like files) to/from gzip base64 representation for easier packing and transport. |
| Generic | CSV | [78](#78) | Given a BOM CSV with multiple fields (7 in total) return a new CSV with just 3 of the original fields. |
| Generic | Commands | [79](#79) | Given an input array with phone numbers will run parallel output commands, calling ojob.io/telco/phoneNumber, for each entry effectively building an output from those multiple command executions. |
| Generic | End of life | [80](#80) | List the versions of a given product with the corresponding end-of-life dates (using endofline.date API) |
| Generic | Excel | [81](#81) | Building an Excel file with the AWS IPv4 and IPv6 ranges (1). |
| Generic | Excel | [82](#82) | Building an Excel file with the AWS IPv4 and IPv6 ranges (2). |
| Generic | Excel | [83](#83) | Building an Excel file with the AWS IPv4 and IPv6 ranges (3). |
| Generic | Excel | [84](#84) | Processes each json file in /some/data creating and updating the data.xlsx file with a sheet for each file. |
| Generic | Excel | [85](#85) | Store and retrieve data from an Excel spreadsheet |
| Generic | HTML | [86](#86) | Generate a HTML with table of emoticons/emojis by category, group, name, unicode and html code. |
| Generic | HTML | [87](#87) | Given an input file, in a specific language (e.g. yaml, json, bash, etc...), output an HTML representation with syntax highlighting. |
| Generic | Hex | [88](#88) | Outputs an hexadecimal representation of the characters of the file provided allowing to adjust how many per line/row. |
| Generic | JWT | [89](#89) | Generates an output JWT (JSON Web Token) given the provided input claims signed with a provided secret. |
| Generic | JWT | [90](#90) | Given an input JWT (JSON Web Token) converts it to a human readable format. |
| Generic | List files | [91](#91) | After listing all files and folders recursively producing a count table by file extension. |
| Generic | RSS | [92](#92) | Builds an HTML file with the current linked news titles, publication date and source from Google News RSS. |
| Generic | RSS | [93](#93) | Example of generating a HTML list of titles, links and publication dates from a RSS feed |
| Generic | RSS | [94](#94) | Generates a HTML page with the current news from Google News, order by date, and opens a browser with it. |
| Generic | RSS | [95](#95) | Parses the Slashdot&#x27;s RSS feed news into a quick clickable HTML page in a browser |
| Generic | Set | [96](#96) | Given two json files, with arrays of component versions, generate a table with the difference on one of the sides. |
| Generic | Set | [97](#97) | Given two json files, with arrays of component versions, generate a table with the union of the two sides. |
| Generic | Template | [98](#98) | Given a meal name will search &#x27;The Meal DB&#x27; site for the corresponding recipe and render a markdown HTML of the corresponding recipe. |
| Generic | Text | [99](#99) | Get a json with lyrics of a song. |
| Generic | Text | [100](#100) | Search a word in the English dictionary returning phonetic, meanings, synonyms, antonyms, etc. |
| Generic | URL | [101](#101) | Given an URL to a resource on a website determine how long ago is was modified given the data provided by the server. |
| Generic | YAML | [102](#102) | Given an YAML file with a data array composed of maps with fields &#x27;c&#x27;, &#x27;s&#x27;, &#x27;d&#x27; and &#x27;e&#x27; filter by any record where any field doesn&#x27;t have contents. |
| GitHub | GIST | [103](#103) | Using GitHub&#x27;s GIST functionality retrieves and parses an oAFp examples YAML file with the template and the corresponding data. |
| GitHub | Releases | [104](#104) | Builds a table of GitHub project releases |
| GitHub | Releases | [105](#105) | Parses the latest GitHub project release markdown notes |
| Grid | Java | [106](#106) | Parses a Java hsperf data + the current rss java process memory into a looping grid. |
| Grid | Java | [107](#107) | Parses a Java hsperf data into a looping grid. |
| Grid | Kubernetes | [108](#108) | Displays a continuous updating grid with a line chart with the number of CPU throtlles and bursts recorded in the Linux cgroup cpu stats of a container running in Kubernetes and the source cpu.stats data |
| Grid | Mac | [109](#109) | Shows a grid with the Mac network metrics and 4 charts for in, out packets and in, out bytes |
| Grid | Mac | [110](#110) | Shows a grid with the Mac storage metrics and 4 charts for read, write IOPS and read, write bytes per second |
| Grid | Unix | [111](#111) | On an Unix/Linux system supporting &#x27;ps&#x27; output formats %cpu and %mem, will output a chart with the percentage of cpu and memory usage of a provided pid (e.g. 12345) |
| JSON Schemas | Lists | [112](#112) | Get a list of JSON schemas from Schema Store catalog |
| Java | Certificates | [113](#113) | Given a Java keystore will obtain a list of certificates and output them order by the ones that will expire first. |
| Kubernetes | Containers | [114](#114) | Parse the Linux cgroup cpu stats on a container running in Kubernetes |
| Kubernetes | Kubectl | [115](#115) | Build an output table with Kubernetes pods with namespace, pod name, container name and corresponding resources using kubectl |
| Kubernetes | Kubectl | [116](#116) | Build an output table with Kubernetes pods with node, namespace, pod name, container name and corresponding resources using kubectl |
| Kubernetes | Kubectl | [117](#117) | Executes a recursive file list find command in a specific pod, namespace and path converting the result into a table. |
| Kubernetes | Kubectl | [118](#118) | Given the list of all Kubernetes objects will produce a list of objects per namespace, kind, apiVersiom, creation timestamp, name and owner. |
| Kubernetes | Kubectl | [119](#119) | List of Kubernetes CPU, memory and storage stats per node using kubectl |
| Kubernetes | Kubectl | [120](#120) | List of Kubernetes pods per namespace and kind using kubectl |
| Kubernetes | Kubectl | [121](#121) | Produces a list of pods&#x27; containers per namespace with the corresponding images and assigned nodes. |
| Kubernetes | PVC | [122](#122) | Produces a table with all Kubernetes persistent volume claims (PVCs) in use by pods. |
| Mac | Activity | [123](#123) | Uses the Mac terminal command &#x27;last&#x27; output to build an activity table with user, tty, from, login-time and logout-time |
| Mac | Brew | [124](#124) | List all the packages and corresponding versions installed in a Mac by brew. |
| Mac | Chart | [125](#125) | On a Mac OS produce a looping chart with the total percentage of current CPU usage. |
| Mac | Info | [126](#126) | Get a list of the current logged users in Mac OS |
| Mac | Info | [127](#127) | Parses the current Mac OS hardware information |
| Mac | Info | [128](#128) | Parses the current Mac OS overview information |
| Mac | Safari | [129](#129) | Get a list of all Mac OS Safari bookmarks into a CSV file. |
| Mac | Tunnelblink | [130](#130) | In a Mac OS with Tunnelblink, if you want to copy all your OpenVPN configurations into ovpn files. |
| Markdown | Tables | [131](#131) | For an input markdown file, parse all tables, transform it to JSON and output as a colored table |
| Network | ASN | [132](#132) | Retrieve an IP to ASN list list and converts it to ndjson |
| Network | ASN | [133](#133) | Retrieve the list of ASN number and names from RIPE and transforms it to a CSV. |
| Network | Latency | [134](#134) | Given a host and a port will display a continuously updating line chart with network latency, in ms, between the current device and the target host and port |
| Ollama | List models | [135](#135) | Parses the list of models currently in an Ollama deployment |
| OpenAF | Channels | [136](#136) | Copy the json result of a command into an etcd database using OpenAF&#x27;s channels |
| OpenAF | Channels | [137](#137) | Getting all data stored in an etcd database using OpenAF&#x27;s channels |
| OpenAF | Channels | [138](#138) | Given a Prometheus database will query for a specific metric (go_memstats_alloc_bytes), during a defined period, every 5 seconds (step) will produce a static chart with the corresponding metric values. |
| OpenAF | Channels | [139](#139) | Perform a query to a metric &amp; label, with a start and end time, to a Prometheus server using OpenAF&#x27;s channels |
| OpenAF | Channels | [140](#140) | Retrieve all keys stores in a H2 MVStore file using OpenAF&#x27;s channels |
| OpenAF | Channels | [141](#141) | Store and retrieve data from a Redis database |
| OpenAF | Channels | [142](#142) | Store and retrieve data from a RocksDB database |
| OpenAF | Channels | [143](#143) | Store the json results of a command into a H2 MVStore file using OpenAF&#x27;s channels |
| OpenAF | Flags | [144](#144) | List the current values of OpenAF/oAFp internal flags |
| OpenAF | Network | [145](#145) | Gets all the DNS host addresses for a provided domain and ensures that the output is always a list |
| OpenAF | Network | [146](#146) | List all MX (mail servers) network addresses from the current DNS server for a hostname using OpenAF |
| OpenAF | Network | [147](#147) | List all network addresses returned from the current DNS server for a hostname using OpenAF |
| OpenAF | OS | [148](#148) | Current OS information visible to OpenAF |
| OpenAF | OS | [149](#149) | Using OpenAF parse the current environment variables |
| OpenAF | OpenVPN | [150](#150) | Using OpenAF code to perform a more complex parsing of the OpenVPN status data running on an OpenVPN container (nmaguiar/openvpn) called &#x27;openvpn&#x27; |
| OpenAF | SFTP | [151](#151) | Generates a file list with filepath, size, permissions, create and last modified time from a SFTP connection with user and password |
| OpenAF | SFTP | [152](#152) | Generates a file list with filepath, size, permissions, create and last modified time from a SFTP connection with user, private key and password |
| OpenAF | TLS | [153](#153) | List the TLS certificates of a target host with a sorted alternative names using OpenAF |
| OpenAF | oJob.io | [154](#154) | Parses ojob.io/news results into a clickable news title HMTL page. |
| OpenAF | oJob.io | [155](#155) | Retrieves the list of oJob.io&#x27;s jobs and filters which start by &#x27;ojob.io/news&#x27; to display them in a rectangle |
| OpenAF | oPacks | [156](#156) | Listing all currently accessible OpenAF&#x27;s oPacks |
| OpenAF | oafp | [157](#157) | Filter the OpenAF&#x27;s oafp examples list by a specific word in the description |
| OpenAF | oafp | [158](#158) | List the OpenAF&#x27;s oafp examples by category, sub-category and description |
| OpenAF | oafp | [159](#159) | Produce a colored table with all the current oafp input and output formats supported. |
| OpenVPN | List | [160](#160) | When using the container nmaguiar/openvpn it&#x27;s possible to convert the list of all clients order by expiration/end date |
| Unix | Activity | [161](#161) | Uses the Linux command &#x27;last&#x27; output to build a table with user, tty, from and period of activity for Debian based Linuxs |
| Unix | Activity | [162](#162) | Uses the Linux command &#x27;last&#x27; output to build a table with user, tty, from and period of activity for RedHat based Linuxs |
| Unix | Alpine | [163](#163) | List all installed packages in an Alpine system |
| Unix | Ask | [164](#164) | Unix bash script to ask for a path and choose between filetypes to perform an unix find command. |
| Unix | Compute | [165](#165) | Parses the Linux /proc/cpuinfo into an array |
| Unix | Debian/Ubuntu | [166](#166) | List all installed packages in a Debian/Ubuntu system |
| Unix | Envs | [167](#167) | Converts the Linux envs command result into a table of environment variables and corresponding values |
| Unix | Files | [168](#168) | Converting the Linux&#x27;s /etc/os-release to SQL insert statements. |
| Unix | Files | [169](#169) | Converting the Unix&#x27;s syslog into a json output. |
| Unix | Files | [170](#170) | Executes a recursive file list find command converting the result into a table. |
| Unix | Files | [171](#171) | Parses the Linux /etc/passwd to a table order by uid and gid. |
| Unix | Generic | [172](#172) | Creates, in unix, a data.ndjson file where each record is formatted from json files in /some/data |
| Unix | Memory map | [173](#173) | Given an Unix process will output a table with process&#x27;s components memory address, size in bytes, permissions and owner |
| Unix | Network | [174](#174) | Loop over the current Linux active network connections |
| Unix | Network | [175](#175) | Parse the Linux &#x27;arp&#x27; command output |
| Unix | Network | [176](#176) | Parse the Linux &#x27;ip tcp_metrics&#x27; command |
| Unix | Network | [177](#177) | Parse the result of the Linux route command |
| Unix | OpenSuse | [178](#178) | List all installed packages in an OpenSuse system or zypper based system |
| Unix | RedHat | [179](#179) | List all installed packages in a RedHat system or rpm based system (use rpm --querytags to list all fields available) |
| Unix | Storage | [180](#180) | Converting the Unix&#x27;s df output |
| Unix | Storage | [181](#181) | Parses the result of the Unix ls command |
| Unix | SystemCtl | [182](#182) | Converting the Unix&#x27;s systemctl list-timers |
| Unix | SystemCtl | [183](#183) | Converting the Unix&#x27;s systemctl list-units |
| Unix | SystemCtl | [184](#184) | Converting the Unix&#x27;s systemctl list-units into an overview table |
| Unix | UBI | [185](#185) | List all installed packages in an UBI system |
| Unix | named | [186](#186) | Converts a Linux&#x27;s named log, for client queries, into a CSV |
| Unix | strace | [187](#187) | Given a strace unix command will produce a summary table of the system calls invoked including a small line chart of the percentage of time of each. |
| Windows | Network | [188](#188) | Output a table with the current route table using Windows&#x27; PowerShell |
| Windows | Network | [189](#189) | Output a table with the list of network interfaces using Windows&#x27; PowerShell |
| Windows | PnP | [190](#190) | Output a table with USB/PnP devices using Windows&#x27; PowerShell |
| Windows | Storage | [191](#191) | Output a table with the attached disk information using Windows&#x27; PowerShell |
| XML | Maven | [192](#192) | Given a Maven pom.xml parses the XML content to a colored table ordering by the fields groupId and artifactId. |
